...
var arrayY = []; // array containing the slider's value for each cell
...
{   
    with (mgraphics){
        ...
        for (var i=0;i<_ntabs;i++)
        {
            ...
            // centered rectangles
            rectangle(cellStarts, -1, cellWidth, -2*arrayY[i]);
            fill();
			...
			if (_selectedHandle == (i-1)) {
                ...
                rectangle(cellStarts, -1, 0, -2*arrayY[i]);
                stroke();
            }
        }
    }
...
function onclick(x,y,but,cmd,shift,capslock,option,ctrl) {
    ...
    if (but == 1) {
        ...
        for (var i=0;i<(_ntabs-1);i++){
            if ((posX > (arrayX[i] - 2*pixelsize)) && (posX < (arrayX[i] + 2*pixelsize)))
            { // grab horizontal handle ?
                _selectedHandle = i;
                break;
            }
        }
		// if we clicked but not on a X gridline, update Y value
		if (_selectedHandle == -1) { //update Y value
	            var selectedCell = findcell(posX, arrayX);
            	arrayY[selectedCell] = posY;
    			outlet(0, "y", arrayY);			
		}
    }
...
function ondrag(x,y,but,cmd,shift,capslock,option,ctrl)
{
    ...
    if (but==1){
        ...
        if (_selectedHandle != -1) {
			...
			outlet(0, "x", arrayX.slice(0, -1)); // return X seperator, omitting the last which is always 1

        }
        else { // change the Y value on the current cell
            var selectedCell = findcell(posX, arrayX);
            //var selectedCell = Math.min(_ntabs-1, Math.round((posX*_ntabs)-0.5));
            arrayY[selectedCell] = posY;
			outlet(0, "y", arrayY);
        }
    } else {
...
function ntabs(val){
	...
	// add tab to the list = complicated because we might need to squeeze existing values
...
function y(){
    var newValues = arrayfromargs(arguments);
	for (var i = 0; i < Math.min(_ntabs, newValues.length) ; i++) {
  		arrayY[i] = newValues[i];
	}
    outlet(0, "y", arrayY);
    mgraphics.redraw();
}
